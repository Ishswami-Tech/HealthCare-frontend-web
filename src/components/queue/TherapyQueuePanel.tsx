...
function getStatusColor(status: QueueItem["status"]) {
  switch (status) {
    case "waiting":
...
function getPriorityColor(priority?: QueueItem["priority"]) {
  switch (priority) {
    case "urgent":
...
function calculateWaitTime(checkedInAt: string) {
...
function calculateSessionTime(startedAt: string) {
...
interface QueueItemCardProps {
  item: QueueItem;
  onStatusChange?: ((itemId: string, newStatus: QueueItem["status"]) => void) | undefined;
  onPriorityChange?: ((itemId: string, newPriority: QueueItem["priority"]) => void) | undefined;
}

const QueueItemCard = ({ item, onStatusChange, onPriorityChange }: QueueItemCardProps) => (
...
const TherapyQueue = ({
  ...
  return (
    <div className="gap-y-4">
      ...
      {items.length > 0 ? (
        <div className="gap-y-3" suppressHydrationWarning>
          {items
            .sort((a, b) => {
