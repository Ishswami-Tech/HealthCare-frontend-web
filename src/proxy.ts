...
function resolveProfileCompletionFromUserData(
  ...
  if (typeof userData.isProfileComplete === "boolean")
...
async function fetchBackendProfileCompletion(
  ...
  try {
    ...
    if (typeof payload.data?.isComplete === "boolean")
      return payload.data.isComplete;
    return undefined;
  } catch {
    return undefined;
  }
...
function extractUserDataFromToken(
  accessToken: string,
  ...
  try {
    ...
    logger.warn("Failed to extract user data from token", {
...
export default async function proxy(request: NextRequest) {
  ...
  if (!pathname.startsWith("/(auth)") && !pathname.startsWith("/auth")) {
    const locale =
      request.cookies.get(LANGUAGE_COOKIE_NAME)?.value || DEFAULT_LANGUAGE;
  ...
  const profileCompleteFromUserData =
    resolveProfileCompletionFromUserData(userData);
  ...
  const wsHost =
    normalizeOrigin(process.env.NEXT_PUBLIC_WEBSOCKET_URL) || apiHost;
