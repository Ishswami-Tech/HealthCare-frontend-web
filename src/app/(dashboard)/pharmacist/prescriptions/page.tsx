...
export default function PrescriptionsPage() {
  ...
  const prescriptions = [
    {
      ...
      medicines: [
        { name: "Triphala Churna", dosage: "1 tsp twice daily", quantity: "100g", available: true },
        { name: "Ashwagandha Capsules", dosage: "2 capsules daily", quantity: "60 caps", available: true },
        { name: "Brahmi Ghrita", dosage: "1 tsp at bedtime", quantity: "200ml", available: false }
      ],
      status: "pending",
      priority: "normal",
    ...
    },
    {
      ...
      medicines: [
        { name: "Chyawanprash", dosage: "1 tbsp daily", quantity: "500g", available: true },
        { name: "Arjuna Tablets", dosage: "2 tablets twice daily", quantity: "60 tabs", available: true }
      ],
      status: "preparing",
      priority: "urgent",
    ...
    },
    {
      ...
      medicines: [
        { name: "Saraswatarishta", dosage: "15ml twice daily", quantity: "450ml", available: true },
        { name: "Medhya Rasayana", dosage: "1 tablet daily", quantity: "30 tabs", available: true }
      ],
      status: "prepared",
      priority: "normal",
  ...
  const handoverHistory = [
    {
      ...
      doctorName: "Dr. Priya Sharma",
      handedOverAt: "2025-08-08 2:30 PM",
      medicines: 3,
      status: "completed"
    },
    {
      ...
      doctorName: "Dr. Amit Patel",
      handedOverAt: "2025-08-08 2:15 PM",
      medicines: 2,
      status: "completed"
    }
  ];
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'prepared': return <Package className="w-4 h-4" />;
      case 'completed': return <CheckCircle className="w-4 h-4" />;
      default: return <AlertTriangle className="w-4 h-4" />;
    }
  };

  const PrescriptionCard = ({ prescription, showActions = true }: { prescription: any, showActions?: boolean }) => (
      ...
      <CardContent className="p-6">
        ...
        <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span>Prescribed: {new Date(prescription.prescribedAt).toLocaleString()}</span>
          </div>
          ...
          {prescription.startedAt && (
            <div className="flex items-center gap-2">
              ...
              <span>Started: {new Date(prescription.startedAt).toLocaleString()}</span>
            </div>
          )}
          {prescription.preparedAt && (
            <div className="flex items-center gap-2">
              <CheckCircle className="w-4 h-4 text-gray-500" />
              <span>Prepared: {new Date(prescription.preparedAt).toLocaleString()}</span>
            </div>
          )}
        </div>

        <div className="mb-4">
          <h4 className="font-medium mb-2">Medicines ({prescription.medicines.length})</h4>
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Manage and track prescription preparation</p>
        </div>
      </div>
          ...
          {handoverHistory.length > 0 ? (
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <p className="text-sm font-medium">Handed over: {item.handedOverAt}</p>
                      <p className="text-xs text-gray-500">{item.medicines} medicines</p>
            ...
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <h3 className="text-lg font-semibold text-gray-600 mb-2">No handover history</h3>
                <p className="text-gray-500">Completed prescriptions will appear here.</p>
              </CardContent>
            </Card>
          )}
        </TabsContent>
