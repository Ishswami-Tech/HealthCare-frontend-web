...
import { legalBrand, legalDates, legalContacts, privacySections } from "@/lib/legal/policy-content";
        ...
        <Card className="border-t-4 border-t-primary">
          <CardHeader className="flex flex-col gap-y-4 border-b pb-8">
            ...
            <p className="text-lg leading-relaxed text-muted-foreground">
              Your privacy is critically important to us. {legalBrand.displayName} uses the{" "}
              {legalBrand.platformName} platform/domain for digital appointments, patient workflows,
          ...
          <CardContent className="flex flex-col gap-y-8 px-6 pt-8 sm:px-10">
            {privacySections.map((section) => (
              <section key={section.title} className="flex flex-col gap-y-3">
                <h2 className="flex items-center gap-2 text-xl font-semibold text-foreground">
                  {section.title.includes("Data Security") ? (
                    <Lock className="size-5 text-primary" />
                  ) : section.title.includes("How We Use") ? (
            ...
            <section className="mt-8 rounded-xl border border-primary/20 bg-primary/10 p-6">
              ...
              <p className="text-sm text-muted-foreground">
                For healthcare/privacy requests, contact{" "}
                <a
